...
public class FormattedDocumentDisplay : MonoBehaviour
{
    ...
    private int DisplayText(List<String> lines, int currentLine)
    {
        ...
        for (line = currentLine + 1; line < lines.Count; line++)
        {
            if (lines[line].Length > 0 && lines[line][0] == '$') break;
            Debug.Log("line: "+ line);
            Debug.Log("lines.Count: " + lines.Count);
            Debug.Log("line content: " + lines[line]);
